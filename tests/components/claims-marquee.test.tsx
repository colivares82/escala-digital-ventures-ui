/**
 * ClaimsMarquee tests.
 * Phase 5: ariaLabel is now a required prop.
 */
...
const ARIA_LABEL = sharedContent.accessibility.keyMessages

describe('ClaimsMarquee', () => {
  it('renders with the accessible key-messages label', () => {
    render(<ClaimsMarquee claims={CLAIMS} ariaLabel={ARIA_LABEL} />)
    // The marquee div has aria-label from sharedContent
    expect(
      screen.getByLabelText(ARIA_LABEL),
    ).toBeInTheDocument()
  })
  ...
  it('marks duplicated items as aria-hidden', () => {
    const { container } = render(<ClaimsMarquee claims={CLAIMS} ariaLabel={ARIA_LABEL} />)
