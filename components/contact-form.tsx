...
export function ContactForm({ email, success, initialState = 'default' }: ContactFormProps) {
  const copy = sharedContent.contactForm
  const seededErrors: Errors = initialState === 'error' ? { name: copy.errors.name, company: copy.errors.company, email: copy.errors.emailRequired, blocker: copy.errors.blocker, consent: copy.errors.consent } : {}
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!emailValue) nextErrors.email = copy.errors.emailRequired
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue)) nextErrors.email = copy.errors.emailInvalid
    if (!String(data.get('blocker') ?? '').trim()) nextErrors.blocker = copy.errors.blocker
  ...
  if (submitted) return <div className="contact-success" role="status" tabIndex={-1}><span>{copy.successLabel}</span><p>{success}</p></div>
  ...
  return <form className="contact-form" onSubmit={handleSubmit} noValidate>
    ...
    <div className="contact-consent contact-field--wide">
      ...
      <label htmlFor="contact-consent">{copy.consentPrefix} <a href="/privacidad">{copy.privacyLabel}</a>.</label>
    ...
    {Object.keys(errors).length > 0 && <p className="contact-fallback">{copy.fallback} <a href={`mailto:${email}`}>{email}</a></p>}
  </form>
}
...
function ContactInput({ id, name, label, type = 'text', error }: { id: string; name: ContactField; label: string; type?: string; error?: string }) {
  ...
  return <div className="contact-field"><label htmlFor={id}>{label}</label><input id={id} name={name} type={type} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} />{error && <span className="contact-error" id={errorId}>{error}</span>}</div>
}
