...
export function ProblemSection({ content }: { content: typeof homeContent.problem }) {
  return (
    <section className="mx-auto grid max-w-7xl grid-cols-1 gap-12 px-5 py-24 md:grid-cols-12 md:px-8 md:py-36">
      ...
      <p className="text-base leading-relaxed text-pretty md:col-span-6 md:col-start-7 md:text-lg">
        {content.body}
      </p>
  ...
  return (
    <section className="mx-auto flex max-w-7xl flex-col gap-14 px-5 py-24 md:px-8 md:py-36">
      <div className="grid grid-cols-1 gap-10 md:grid-cols-12">
        ...
        <p className="text-base leading-relaxed md:col-span-5 md:col-start-8 md:self-end md:text-lg">
          {content.description}
        </p>
      </div>
  ...
  return (
    <section className="mx-auto flex max-w-7xl flex-col gap-14 px-5 py-24 md:px-8 md:py-36">
      ...
      <div className="grid grid-cols-1 gap-10 md:grid-cols-12">
        ...
        <p className="font-mono text-xs font-medium text-primary md:col-span-3 md:col-start-10 md:self-end">
          {content.client}
        </p>
      </div>
      <dl className="grid grid-cols-1 border-t border-border sm:grid-cols-2 lg:grid-cols-4">
        {content.figures.map((figure) => (
          <div className="readout flex min-h-48 flex-col justify-between gap-8 border-b border-border py-6 sm:px-6 sm:first:pl-0 lg:border-r lg:last:border-r-0" key={figure.label}>
            <dt className="max-w-44 font-mono text-[0.6875rem] uppercase leading-relaxed tracking-wider text-muted-foreground">
              {figure.label}
            </dt>
            <dd className="font-mono text-[clamp(2.5rem,4vw,4rem)] font-medium leading-none tracking-[-0.04em] text-primary">
              {figure.value}
            </dd>
          </div>
        ))}
