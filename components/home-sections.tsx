...
export function FrameworkSection({ content }: { content:typeof homeContent.framework }) {
 return <section className="section section--dark dark-surface" id="metodo"><div className="page-shell"><SectionIndex index="03" label="EL CICLO DE CRECIMIENTO"/><div className="split-heading"><div><WordReveal text={content.title}/><p className="lead-copy">{content.description}</p></div><a className="text-link" href="/como-trabajamos">{content.action}<span aria-hidden="true">↗</span></a></div><PhaseCycle phases={content.phases}/></div></section>
}
