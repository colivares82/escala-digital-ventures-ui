...
export function ProofSection({ content }: { content:typeof homeContent.proof }) {
 return <section className="section section--light proof" id="casos"><div className="page-shell"><SectionIndex index="04" label="EVIDENCIA"/><div className="split-heading"><div><WordReveal text={content.title}/><p className="proof__attribution">EVIDENCIA VERIFICADA — MAGUPELL (EN PRODUCCIÓN) · BIOZERO (V1 ENTREGADA)</p></div><SystemDiagram kind="proof" label="Evolución verificada de la operación"/></div><dl className="readouts">{content.figures.map((f,i)=><Readout key={f.label} {...f} index={i}/>)}</dl><div className="case-grid">{content.cases.map((item,i)=><CaseStudyCard key={item.name} item={item} index={i}/>)}</div></div></section>
}
